...
- Locale status is not one boolean. Distinguish CSW runtime parity, sibling Skill runtime parity, Method Definition parity, technical-asset localization, research-material localization, and independent review. `research/skill-prototypes/REFERENCE-CLASSIFICATION.md` defines this boundary.
- English sibling `SKILL.en.md` / `METHOD.en.md` files are translated research drafts until independently reviewed. Untranslated evidence/eval/migration prose is not implicit English runtime instruction.
- Run `make research-skill-check` when changing the split-method prototypes or suite manifest. Use `make research-skill-preview` when the assembled three-Skill × two-locale research package needs direct inspection.
...
- When a short-lived branch is used, keep one coherent task on one branch and one pull request. Reuse that branch and pull request for follow-up corrections to the same task instead of creating sibling or replacement branches merely because the work continues in another session or context.
- A short-lived-branch task is not complete until its pull request has been merged back into the branch it started from, normally the active `develop/vX.Y.Z`, and the updated base branch has been verified to contain the result. Immediately before merging, refetch both base and head so concurrent work is not silently overwritten or bypassed. If merging is genuinely blocked, leave one branch and one pull request as the single continuation point, record the blocker explicitly, and do not create a successor branch for the same task.
...
- When Japanese canonical runtime content changes, update the corresponding English translation and `i18n/translation-manifest.json` before release. A research branch may carry a clearly marked `translated-draft` sibling realization, but it must not claim independently reviewed locale parity or public multi-skill distribution parity until the corresponding gates are resolved.
