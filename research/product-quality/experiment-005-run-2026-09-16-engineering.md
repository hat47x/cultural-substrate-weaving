...
生成物は直接編集しない。OpenAI Skillの`dist/`はGit管理外であり、`build_openai()`が各locale / profileのadapter YAMLを`agents/openai.yaml`へそのままコピーする。Gitで追跡する`.claude-plugin/`、`.agents/`、`plugins/`を含め、生成結果そのものではなく、正本となる入力またはbuild処理を修正する。
