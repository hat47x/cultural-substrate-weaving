...
SOURCE_PROJECTION_PREVIEW = Path("research/skill-prototypes/scripts/preview_production_source_projection.py")
SOURCE_PROJECTION_TEST = Path("tests/test_research_production_source_projection_preview.py")
ADAPTER_PROMOTION_PLANNER = Path("research/skill-prototypes/scripts/plan_production_adapter_metadata_promotion.py")
ADAPTER_PROMOTION_TEST = Path("tests/test_research_production_adapter_metadata_promotion.py")
RELEASE_PLAN = Path("research/skill-prototypes/P4-RELEASE-INTERNAL-COMPOSITION-PLAN-2026-09-07.md")
RELEASE_VALIDATOR = Path("scripts/validate_research_release_composition_plan.py")
RELEASE_TEST = Path("tests/test_research_release_composition_plan.py")
...
OBSERVATION_IDS = (
    ...
    "production_adapter_metadata_promotion",
...
def _contract_and_regression_state(
    root: Path,
    contract_path: Path,
    test_path: Path,
    *,
    complete_state: str,
    contract_only_state: str,
) -> tuple[str, list[str]]:
    contract_present = _exists(root, contract_path)
    test_present = _exists(root, test_path)
    evidence = [path.as_posix() for path in (contract_path, test_path) if _exists(root, path)]
    if contract_present and test_present:
        return complete_state, evidence
    if contract_present:
        return contract_only_state, evidence
...
def observe_promotion_readiness(root: Path = ROOT) -> dict:
    ...
    observations.append(
        _obs(
            ...
            details={
                "declared_status": evaluation_status,
                "declared_evaluation_type": evaluation_type,
            },
        )
    )
    ...
    observations.append(
        _obs(
            ...
            notes=[
                "Declared check presence is not a durable execution record.",
                "A make invocation may execute the check before this observer without making the observer the authority for that result.",
            ],
    ...
    handoff_evidence = [
        path.as_posix() for path in (HANDOFF_CASES, HANDOFF_CAPSULE) if _exists(root, path)
    ]
    observations.append(
        _obs(
            "cross_layer_handoff",
    ...
    tension_evidence = [
        path.as_posix() for path in (TENSION_CASES, TENSION_TEST) if _exists(root, path)
    ]
    observations.append(
        _obs(
            "csw_tension_ownership_evaluation",
            ...
            notes=[
                ...
                "Regression source presence and same-run command reachability are not a durable regression execution record.",
            ],
        )
    )
    ...
    host_state = (
        "contract-and-regression-present-execution-unrecorded"
    ...
    observations.append(
        _obs(
            ...
            notes=["Presence of tests is not a durable test-execution record."],
        )
    )
    ...
    observations.append(
        _obs(
            ...
            notes=["Oracle presence and durable oracle execution evidence remain distinct."],
        )
    )
    ...
    source_projection_state, source_projection_evidence = _contract_and_regression_state(
        root,
        SOURCE_PROJECTION_PREVIEW,
        SOURCE_PROJECTION_TEST,
        complete_state="preview-and-regression-present-execution-unrecorded",
        contract_only_state="preview-contract-present-unclassified",
    )
    observations.append(
        _obs(
            "production_source_projection",
            source_projection_state,
            authority=SOURCE_PROJECTION_PREVIEW.as_posix()
            if _exists(root, SOURCE_PROJECTION_PREVIEW)
            else None,
            evidence=source_projection_evidence,
            evidence_kind="read-only-production-source-content-projection",
            notes=[
                "The preview applies declared transforms in memory but does not write production source.",
                "Preview/test source presence and same-run reachability are not a durable preview execution record.",
            ],
        )
    )

    adapter_state, adapter_evidence = _contract_and_regression_state(
        root,
        ADAPTER_PROMOTION_PLANNER,
        ADAPTER_PROMOTION_TEST,
        complete_state="planner-and-regression-present-execution-unrecorded",
        contract_only_state="planner-contract-present-unclassified",
    )
    observations.append(
        _obs(
            "production_adapter_metadata_promotion",
            adapter_state,
            authority=ADAPTER_PROMOTION_PLANNER.as_posix()
            if _exists(root, ADAPTER_PROMOTION_PLANNER)
            else None,
            evidence=adapter_evidence,
            evidence_kind="read-only-production-adapter-metadata-promotion-plan",
            notes=[
                "Adapter metadata promotion planning remains distinct from metadata review and production mutation.",
                "Planner/test source presence and same-run reachability are not a durable execution record.",
            ],
        )
    )
    ...
    builder_present = builder_path is not None and _exists(root, builder_path)
    builder_evidence = [BUILDER_CONTRACT.as_posix()] if design_present else []
    if builder_present and builder_path is not None:
    ...
    observations.append(
        _obs(
            ...
            _design_and_file_state(
                ...
                production_file_present=builder_present,
            ),
            ...
            details={
                ...
                "production_file": builder_path.as_posix() if builder_path else None,
            },
        )
    )

    validator_present = validator_path is not None and _exists(root, validator_path)
    validator_evidence = [BUILDER_CONTRACT.as_posix()] if design_present else []
    if validator_present and validator_path is not None:
    ...
    observations.append(
        _obs(
            ...
            _design_and_file_state(
                design_present=design_present,
                production_file_present=validator_present,
            ),
            authority=BUILDER_CONTRACT.as_posix() if design_present else None,
            ...
            details={
                "design_status": design_status,
                "production_file": validator_path.as_posix() if validator_path else None,
            },
        )
    )
    ...
    observations.append(
        _obs(
            "production_inclusion_descriptor",
            "descriptor-present-execution-unrecorded"
            if inclusion_present
            else "not-observed-in-this-branch",
            authority=planned_descriptor.as_posix()
            if inclusion_present and planned_descriptor
            else None,
            evidence=[planned_descriptor.as_posix()]
            if inclusion_present and planned_descriptor
            else [],
    ...
    release_evidence = [
        path.as_posix()
        for path in (RELEASE_PLAN, RELEASE_VALIDATOR, RELEASE_TEST)
        if _exists(root, path)
    ]
    release_plan_present = _exists(root, RELEASE_PLAN) and isinstance(
        descriptor.get("release_shape"), dict
    )
    release_validator_present = _exists(root, RELEASE_VALIDATOR)
    release_test_present = _exists(root, RELEASE_TEST)
    if release_plan_present and release_validator_present and release_test_present:
        release_state = "design-and-validator-present-execution-unrecorded"
    elif release_plan_present:
        release_state = "design-only"
    elif release_validator_present or release_test_present:
        release_state = "validator-source-present-without-design"
    else:
        release_state = "not-observed-in-this-branch"
    observations.append(
        _obs(
            "release_internal_composition",
            release_state,
            authority=RELEASE_PLAN.as_posix() if release_plan_present else None,
            evidence=release_evidence,
            evidence_kind="release-composition-design-and-regression",
            notes=[
                "A release composition validator/test does not establish a durable validation PASS without an execution record.",
                "Release composition design does not authorize release packaging changes.",
            ],
        )
    )
    ...
    observations.append(
        _obs(
            ...
            notes=[
                "Package prototypes, projection previews, metadata plans, and tests do not establish real-host invocation/routing behavior."
            ],
        )
    )
    ...
    return {
        "schema": "csw.research-promotion-readiness-observation/v4",
