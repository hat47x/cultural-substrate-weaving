...
When comparison or measurement is used, distinguish at least:

1. a directly observable difference;
2. a recorded measurement;
3. the user's own judgment;
4. an interpretation by an AI or external evaluator.

If an AI defines an evaluation axis, scores its own generated output, and explains the difference as evidence for method effectiveness, do not treat that explanation as the measurement itself. The same caution applies when the evaluator is a separate AI chat.

If the user later corrects or withdraws a judgment, preserve that later history and do not reinterpret the older AI evaluation as a user judgment.
...
This check can itself become an AI judgment. Whenever practical, return it to concrete target differences, user corrections, and source references.
