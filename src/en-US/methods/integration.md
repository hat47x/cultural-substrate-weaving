...
Attested KJ practice emphasizes cards readable by a third party, labels that are not category names, and the A-type diagram / B-type narration distinction. **Covering the material before raising a label, card boundaries, provenance-metadata design, the addition patterns used in return checks, and the rerun method for return checks include modern adaptations for generative-AI use.** Do not use card counts or character counts as rules for semantic boundaries.
