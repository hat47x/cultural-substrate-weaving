...
- Extended natural-Japanese prose review freshness from development/maintainer material to the public Japanese README, getting-started and usage guides, and platform guides; refreshed the reviewed prose without changing runtime method semantics, and added the current GitHub-marketplace workspace import path to the Codex guide.
- Corrected Microsoft 365 guidance against the current declarative-agent Instructions contract: Knowledge is treated as factual grounding rather than an extension of agent instructions, the bundled `knowledge/` method modules are no longer presented as a way to bypass the 8,000-character Instructions limit, the current adapter is documented as limited to the instructions carried by `instructions.txt`, and full adapter redesign is tracked in Issue #96. Also clarified that Agent Builder's 20-item limit applies to files uploaded directly from the device as embedded knowledge sources.
