...
class GenericSkillValidationHelperTests(unittest.TestCase):
    ...
    def test_skill_entry_budget_remains_byte_based_for_multibyte_text(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            skill_md = Path(temp_dir) / "SKILL.md"
            skill_md.write_text("あ", encoding="utf-8")
            errors: list[str] = []
            ...
            size = check_skill_entry_budget(
                ...
                skill_md=skill_md,
                max_bytes=2,
                label="ja-JP Skill SKILL.md",
            )

            self.assertEqual(size, len("あ".encode("utf-8")))
            self.assertEqual(
                errors,
                [f"ja-JP Skill SKILL.md exceeds budget: {len('あ'.encode('utf-8'))}"],
            )
