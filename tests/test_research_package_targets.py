...
class ResearchPackageTargetTests(unittest.TestCase):
    ...
    def test_realized_locale_requires_package_targets(self) -> None:
        ...
        affinity["locale_realizations"]["ja-JP"].pop("package_targets")

        self.assert_has_error(manifest, "realized locale ja-JP must declare package_targets")
    ...
    def test_planned_locale_cannot_carry_stale_package_targets(self) -> None:
        ...
        iterative["locale_realizations"]["en-US"]["package_targets"] = copy.deepcopy(
            iterative["locale_realizations"]["ja-JP"]["package_targets"]
        )
    ...
    def test_package_targets_must_cover_skill_tree_distributions_exactly(self) -> None:
        ...
        self.assert_has_error(
            manifest,
            "package_targets must match Skill-tree distributions; missing=['codex_plugin']",
        )

    def test_unsafe_target_name_is_rejected(self) -> None:
        manifest = copy.deepcopy(self.manifest)
        iterative = self.skill(manifest, "iterative-inquiry-synthesis")
        iterative["locale_realizations"]["ja-JP"]["package_targets"]["openai_skill"][
            "skill_name"
    ...
    def test_target_name_collision_is_rejected(self) -> None:
        ...
        affinity = self.skill(manifest, "affinity-synthesis")
        iterative = self.skill(manifest, "iterative-inquiry-synthesis")
        iterative["locale_realizations"]["ja-JP"]["package_targets"]["claude_plugin"][
            "skill_name"
        ...
        for skill in self.manifest["skills"]:
            for locale, realization in skill["locale_realizations"].items():
                if realization["status"] == "planned":
                    continue
                self.assertEqual(
                    realization["package_targets"]["openai_skill"]["skill_name"],
                    skill["installable_name"],
                    f"{skill['id']} {locale}",
                )
    ...
    def test_csw_claude_and_codex_targets_match_existing_adapter(self) -> None:
        ...
        for locale, adapter in adapter_locales.items():
            ...
            self.assertEqual(
                realization["package_targets"]["claude_plugin"]["skill_name"],
                expected,
            )
            self.assertEqual(
                realization["package_targets"]["codex_plugin"]["skill_name"],
                expected,
            )
