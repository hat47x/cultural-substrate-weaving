...
class ReleaseLifecycleContractTests(unittest.TestCase):
    ...
    def test_localized_release_procedures_keep_the_same_publication_gates(self) -> None:
        ...
        self.assertIn("指定したタグ名が最終manifestの版と一致", ja)
        self.assertIn("source_commit`がリポジトリ方針どおり2つの親を持つマージコミットの形", ja)
        self.assertIn("ここで確認する2親形状も", ja)
        self.assertIn("現在のリモート`main`履歴", ja)
        self.assertIn("CHANGELOG公開境界", ja)
        ...
        self.assertIn("remote tag name to match the final manifest version", en)
        self.assertIn("source commit still has the expected two-parent merge-commit shape", en)
        self.assertIn("current remote `main` history", en)
        self.assertIn("CHANGELOG boundary remains frozen", en)
    ...
    def test_remote_release_verification_remains_a_manual_publication_gate(self) -> None:
        ...
        self.assertIn("final manifest `source_commit` to have exactly two parents", text)
        self.assertIn("final manifest `source_commit` to be present in current remote `main` history", text)
        self.assertIn("CHANGELOG boundary for the manifest version to remain frozen", text)
        self.assertIn("two-parent shape still does not prove pull-request provenance", text)
