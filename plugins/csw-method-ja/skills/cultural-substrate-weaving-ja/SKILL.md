...
3. **適用範囲を選ぶ**：通常手法／限定適用／全体適用のいずれかを選ぶ。**判断が拮抗する場合は限定適用から始めて測る。** 明示指定を受けた場合は、通常手法で足りると見えても限定適用は行う。
4. **必要な参照だけを読む**：各参照から得たい情報を一文で定義し、目的を説明できるファイルだけを読む。
5. **探索・検証・変換を行う**：増分の経路は二つある。**体系から構造候補を取得する**か、**対象の材料そのものを統合して空白を見つける**か。材料が多数・異種、または主要構造が未確定なら後者を先に行う。どちらも対象側で採否を決め、対象固有の表現・設計・手順へ変換する。
6. **対象側の増分で止める**：抽象度や説明量だけが増え、新しい事実・関係・条件・反証・判断が生じない場合は終了する。**周回する場合は、結論が動いたかで見る。根拠だけが増えて中心の言明が同じなら、それは補強であって周回ではない。**
...
| 多数の断片の統合、カード化、空白の検出 | [10-integration.md](references/10-integration.md) |

### 読み込み順

- **限定適用**：対象に直接必要な参照を一つ選んで開始する。判断軸の詳細が必要な場合だけ`00-principles-and-constraints.md`を追加する。
- **全体適用**：`00-principles-and-constraints.md`と`01-scope-and-facts.md`から始める。材料が多数・異種、または主要構造が未確定の場合は`10-integration.md`を先に読む。体系選定時に`02-system-selection.md`、体系を採用したら`02a-framework-application.md`、変換時に`03-transformation.md`、確定前に`09-evaluation-and-domains.md`を読む。
