...
**適用先は、探索空間が開いていて到達点が未定の対象である。** 閉じた問題では通常手法を優先する。ただし明示指定があれば、限定適用して基準線との差分を測る。

## 最小実行手順

1. **基準線と保持事項を置く**：領域固有手法だけの実出力と、壊してはならない対象固有の事実・長所・例外を確認する。
2. **発動範囲を選ぶ**：非発動／限定適用／全体適用を選ぶ。拮抗時は低コストな限定適用で測る。
...
| 対象範囲、基準線、保持事項、事実整理 | [01-scope-and-facts.md](references/01-scope-and-facts.md) |
