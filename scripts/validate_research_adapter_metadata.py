...
def _validate_openai(root: Path, config: dict, skill_ids: set[str], locales: set[str], errors: list[str]) -> None:
    ...
    for profile, profile_config in profiles.items():
        ...
        if not isinstance(expected_implicit, bool):
            errors.append(f"openai_skill profile {profile} expected_allow_implicit_invocation must be boolean")
    ...
    if set(skills) != skill_ids:
        errors.append("openai_skill adapter metadata skill ids must match suite skills; " + _set_mismatch(skill_ids, set(skills)))

    required_markers = ("interface:", "display_name:", "short_description:", "default_prompt:", "policy:")
    ...
    for skill_id in sorted(skill_ids & set(skills)):
        ...
        if set(locale_map) != locales:
            errors.append(f"openai_skill metadata locales for {skill_id} must match suite locales; " + _set_mismatch(locales, set(locale_map)))
        ...
        for locale in sorted(locales & set(locale_map)):
            ...
            if set(profile_map) != profile_names:
                errors.append(f"openai_skill metadata profiles for {skill_id}/{locale} must match profiles; " + _set_mismatch(profile_names, set(profile_map)))

            for profile in sorted(profile_names & set(profile_map)):
                entry = profile_map.get(profile)
                if not isinstance(entry, dict):
                    errors.append(f"openai_skill metadata entry {skill_id}/{locale}/{profile} must be an object")
                    continue
                status = entry.get("status")
                if status not in ALLOWED_SOURCE_STATUS:
                    errors.append(f"openai_skill metadata entry {skill_id}/{locale}/{profile} has invalid status: {status!r}")
                    continue

                source_relative = entry.get("source")
                if status == "planned":
                    if source_relative is not None:
                        errors.append(f"planned openai_skill metadata {skill_id}/{locale}/{profile} must not declare source")
                    continue

                source = _repo_path(root, source_relative, f"openai_skill metadata source {skill_id}/{locale}/{profile}", errors)
                if source is None:
                    continue
                if not source.is_file():
                    errors.append(f"openai_skill metadata source is missing for {skill_id}/{locale}/{profile}: {source_relative}")
                    continue
                text = source.read_text(encoding="utf-8")
                for marker in required_markers:
                    if marker not in text:
                        errors.append(f"openai_skill metadata source {skill_id}/{locale}/{profile} is missing marker {marker!r}")
                profile_config = profiles.get(profile)
                if isinstance(profile_config, dict):
                    expected_implicit = profile_config.get("expected_allow_implicit_invocation")
                    if isinstance(expected_implicit, bool):
                        expected_line = "allow_implicit_invocation: true" if expected_implicit else "allow_implicit_invocation: false"
                        if expected_line not in text:
                            errors.append(f"openai_skill metadata source {skill_id}/{locale}/{profile} must contain {expected_line!r}")


def _validate_locale_bundle(root: Path, distribution_name: str, suite_distribution: dict, config: dict, locales: set[str], errors: list[str]) -> None:
    ...
    source = _repo_path(root, source_relative, f"{distribution_name} adapter metadata source", errors)
    catalog = None
    if source is not None:
        if not source.is_file():
            errors.append(f"{distribution_name} adapter metadata source is missing: {source_relative}")
        else:
            catalog = _load_json(source, f"{distribution_name} adapter metadata source", errors)
    ...
    if set(locale_states) != locales:
        errors.append(f"{distribution_name} adapter metadata locales must match suite locales; " + _set_mismatch(locales, set(locale_states)))

    multi_skill = len(suite_distribution.get("contains", [])) > 1
    review_required = config.get("review_required_for_multi_skill")
    if not isinstance(review_required, bool):
        errors.append(f"{distribution_name} review_required_for_multi_skill must be boolean")
    ...
    for locale in sorted(locales & set(locale_states)):
        ...
        if status not in ALLOWED_BUNDLE_STATUS:
            errors.append(f"{distribution_name} metadata locale {locale} has invalid status: {status!r}")
            continue
        if multi_skill and status == "existing-baseline" and not review_required:
            errors.append(f"{distribution_name} multi-Skill bundle using existing-baseline metadata must require review")

        if status == "planned" or catalog is None:
            continue
        catalog_entry = catalog.get(locale)
        if not isinstance(catalog_entry, dict):
            errors.append(f"{distribution_name} locale catalog does not declare metadata for {locale}")
            continue
        for field in ("plugin_name", "skill_name", "description", "display"):
            value = catalog_entry.get(field)
            if not isinstance(value, str) or not value.strip():
                errors.append(f"{distribution_name} locale catalog {locale} must declare {field}")


def validate_adapter_metadata(root: Path, plan: dict) -> list[str]:
    errors: list[str] = []
    if plan.get("schema") != EXPECTED_SCHEMA:
        errors.append(f"research adapter metadata schema must be {EXPECTED_SCHEMA}: {plan.get('schema')!r}")
    ...
    if not isinstance(skills, list) or not isinstance(locales_obj, dict) or not isinstance(distributions, dict):
        return errors + ["suite manifest shape is invalid for adapter metadata validation"]

    skill_ids = {skill.get("id") for skill in skills if isinstance(skill, dict) and isinstance(skill.get("id"), str)}
    locales = set(locales_obj)
    skill_tree_distributions = {name for name, config in distributions.items() if isinstance(config, dict) and config.get("mode") in SKILL_TREE_MODES}
    ...
    if set(metadata_distributions) != skill_tree_distributions:
        errors.append("adapter metadata distributions must match Skill-tree distributions; " + _set_mismatch(skill_tree_distributions, set(metadata_distributions)))
    ...
    for distribution_name in ("claude_plugin", "codex_plugin"):
        ...
        if isinstance(config, dict) and isinstance(suite_distribution, dict):
            _validate_locale_bundle(root, distribution_name, suite_distribution, config, locales, errors)
