        ...
        for locale in locales:
            ...
            if status == "planned":
                if targets is not None:
                    errors.append(
                        f"skill {skill_id}: planned locale {locale} must not declare package_targets"
                    )
                continue

            if not isinstance(targets, dict):
                errors.append(
                    f"skill {skill_id}: realized locale {locale} must declare package_targets"
                )
                continue
            ...
            if missing or extra:
                errors.append(
                    f"skill {skill_id}: locale {locale} package_targets must match Skill-tree "
                    f"distributions; missing={missing}, extra={extra}"
                )
            ...
            for distribution_name in sorted(expected & actual):
                ...
                if not isinstance(target, dict):
                    errors.append(
                        f"skill {skill_id}: locale {locale} package target "
                        f"{distribution_name} must be an object"
                    )
                    continue
                ...
                if not _safe_skill_name(skill_name):
                    errors.append(
                        f"skill {skill_id}: locale {locale} package target "
                        f"{distribution_name} has unsafe skill_name: {skill_name!r}"
                    )

    # Detect collisions within the namespace actually shared by each distribution.
    for locale in locales:
        for distribution_name, config in manifest.get("distribution_prototypes", {}).items():
            ...
            if mode == "standalone_per_skill":
                ...
                target_skill_ids = [
                    skill_id
                    for skill_id in config.get("contains", [])
            ...
            for skill_name, owners in sorted(by_name.items()):
                if len(owners) > 1:
                    errors.append(
                        f"locale {locale} distribution {distribution_name} target skill_name "
                        f"collision {skill_name!r}: {owners}"
                    )
